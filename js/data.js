...
const Store = {
  ...
  /* ---------- Community discussion forum (separate from direct messages) ---------- */
  ...
  async getCommunityPosts(category) {
    ...
    // Reply counts, one query for all posts on the page rather than N+1.
    ...
    return data.map((p) => {
      ...
      return {
        id: p.id, authorId: p.author_id, category: p.category, title: p.title, body: p.body,
        votes: p.votes, createdAt: p.created_at, replyCount: replyCounts[p.id] || 0,
      };
    });
  },
  ...
  async getCommunityPost(id) {
    const { data, error } = await sb
      ...
      .order("created_at", { ascending: true });
    if (error) throw error;
    data.forEach((p) => { if (p.author) this._cache.profileById.set(p.author.id, mapProfile(p.author)); });

    const mapped = data.map((p) => ({
      id: p.id, authorId: p.author_id, parentId: p.parent_id, category: p.category,
      title: p.title, body: p.body, votes: p.votes, createdAt: p.created_at,
    }));
    const post = mapped.find((p) => p.id === id);
    const replies = mapped.filter((p) => p.parentId === id);
    return post ? { ...post, replies } : null;
  },

  async postCommunityPost({ category, title, body }) {
    const user = this.getUser();
    if (!user) throw new Error("Must be signed in to post");
    const { data, error } = await sb
      .from("community_posts")
  ...
  async postCommunityReply(postId, body) {
    ...
    const { error } = await sb.from("community_posts").insert({ author_id: user.id, parent_id: postId, body });
    if (error) throw error;
  },
